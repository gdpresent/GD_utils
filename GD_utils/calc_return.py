...
def calc_return(ratio_df, cost, n_days_after=0):
    return return_calculator(ratio_df, cost, n_days_after).portfolio_cumulative_return
def calc_return_contribution(ratio_df, cost, n_days_after=0):
    return retcnt_calculator(ratio_df, cost, n_days_after).stockwise_period_return_contribution

def calc_return_old(ratio_df, cost, n_days_after=0):
    return return_calculator_v1(ratio_df, cost, n_days_after).backtest_cumulative_return
def calc_return_contribution_old(ratio_df, cost, n_days_after=0):
    return return_calculator_v1(ratio_df, cost, n_days_after).daily_ret_cntrbtn
