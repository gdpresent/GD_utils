...
def get_US_macro(today_not_str=pd.Timestamp.now()):
    ...
    return DATA
def get_US_macro_except_cif(today_not_str=pd.Timestamp.now()):
    ...
    # release: 익월 중순 15일 전후 -> 익월말일에 최초관측가능하다고 하자
    # CPI = data[("USA",'CPALTT01','IXOBSA')].rename("CPI")
    CPI = pdr.get_data_fred('CPIAUCSL',start='1955-01-01', end=today_not_str.strftime('%Y-%m-%d')).rename(columns={'CPIAUCSL': 'CPI'}).rename_axis('date', axis=0)
    CPI.loc[CPI.index[-1] + pd.DateOffset(months=1)] = CPI.iloc[-1]
    CPI = CPI.shift(1).dropna()
    CPI = pd.concat([CPI, End_Month_Dates], axis=1).fillna(method='ffill').groupby(pd.Grouper(freq='BM')).tail(1).drop('SNP500', axis=1)['CPI']
...
def get_KRmacro_data(your_key):
    ...
    KRBond_2Y = get_BOK_macro('금리', '국고채2Y', your_key).rename(columns={"국고채2Y": 'KRBond2Y'}) #daily
    ...
    KRW_USD = get_BOK_macro('환율', '원달러', your_key).rename(columns={"한국": 'KRW_USD'}) #daily
    KRW_YEB = get_BOK_macro('환율', '원엔', your_key).rename(columns={"한국": 'KRW_YEN'}) #daily
...
def get_economic_schedule(date_target=None):
    ...
    if len(bs_obj_read)>0:
        ...
        objs = bs_obj_read[0].find_all('li')
